package com.cattle.component.kettle.trans;
...
import static com.cattle.component.kettle.trans.MyTrans.BitMaskStatus.FINISHED;
import static com.cattle.component.kettle.trans.MyTrans.BitMaskStatus.RUNNING;

/**
 * @author lsj
 * 原保存步骤日志 会生成新的batchId 但并发执行此方法时会生成重复ID
 * 所以需要修改batchId生成部分，如果设置了batchid 在步骤日志中保存则不会重新生成batchid
 * 新增部分
 * 2114行
 */
public class MyTrans extends Trans {
     ...
     */
    public MyTrans( TransMeta transMeta ) {
        this( transMeta, null );
    }
     ...
     */
    public MyTrans( TransMeta transMeta, LoggingObjectInterface parent ) {
        this();
     ...
     */
    public <Parent extends VariableSpace & NamedParams> MyTrans( Parent parent, Repository rep, String name, String dirname,
    ...
    public void prepareExecution( String[] arguments ) throws KettleException {
        ...
        for ( int i = 0; i < initThreads.length; i++ ) {
            ...
            if ( !initThreads[ i ].isOk() ) {
                ...
                combi.data.setStatus( BaseStepData.StepExecutionStatus.STATUS_IDLE );
                if ( log.isDetailed() ) {
        ...
        if ( !ok ) {
            ...
            for ( int i = 0; i < initThreads.length; i++ ) {
                ...
                if ( initThreads[ i ].isOk() ) {
                    combi.data.setStatus( BaseStepData.StepExecutionStatus.STATUS_HALTED );
                } else {
                    combi.data.setStatus( BaseStepData.StepExecutionStatus.STATUS_STOPPED );
                }
            }
                    ...
                    if ( nrOfActiveSteps == 1 ) {
                        ...
                        synchronized ( transListeners ) {
                            for ( TransListener listener : transListeners ) {
                                listener.transActive( MyTrans.this );
                            }
                        }
                    }
                }
    ...
    public int getEnded() {
        ...
        for ( int i = 0; i < steps.size(); i++ ) {
            ...
            if ( ( sid.step != null && !sid.step.isRunning() )
                    ...
                    || data.getStatus() == BaseStepData.StepExecutionStatus.STATUS_FINISHED || // Finished processing
                    data.getStatus() == BaseStepData.StepExecutionStatus.STATUS_HALTED || // Not launching because of init error
                    data.getStatus() == BaseStepData.StepExecutionStatus.STATUS_STOPPED ) { // Stopped because of an error
                nrEnded++;
            }
        }
    ...
    protected void setFinished( boolean finished ) {
        status.updateAndGet( v -> finished ? v | FINISHED.mask : ( BitMaskStatus.BIT_STATUS_SUM ^ FINISHED.mask ) & v );
    }
    ...
    public int nrActiveSteps() {
        ...
        for ( int i = 0; i < steps.size(); i++ ) {
            ...
            if ( sid.step.isRunning() || sid.step.getStatus() != BaseStepData.StepExecutionStatus.STATUS_FINISHED ) {
                nr++;
            }
        }
        return nr;
    ...
    public boolean hasHaltedSteps() {
        ...
        for ( int i = 0; i < steps.size(); i++ ) {
            StepMetaDataCombi sid = steps.get( i );
            if ( sid.data.getStatus() == BaseStepData.StepExecutionStatus.STATUS_HALTED ) {
                return true;
            }
        }
        return false;
    ...
    public boolean isInitializing() {
        int exist = status.get() & BitMaskStatus.INITIALIZING.mask;
        return exist != 0;
    }
    ...
    public void setInitializing( boolean initializing ) {
        status.updateAndGet( v -> initializing ? v | BitMaskStatus.INITIALIZING.mask : ( BitMaskStatus.BIT_STATUS_SUM ^ BitMaskStatus.INITIALIZING.mask ) & v );
    }
    ...
    public boolean isPreparing() {
        int exist = status.get() & BitMaskStatus.PREPARING.mask;
        return exist != 0;
    }
    ...
    public void setPreparing( boolean preparing ) {
        status.updateAndGet( v -> preparing ? v | BitMaskStatus.PREPARING.mask : ( BitMaskStatus.BIT_STATUS_SUM ^ BitMaskStatus.PREPARING.mask ) & v );
    }
    ...
    public void setRunning( boolean running ) {
        status.updateAndGet( v -> running ? v | RUNNING.mask : ( BitMaskStatus.BIT_STATUS_SUM ^ RUNNING.mask ) & v );
    }
     ...
     * @see org.pentaho.di.core.variables.VariableSpace#environmentSubstitute(String)
     */
    @Override
     ...
     * @see org.pentaho.di.core.variables.VariableSpace#getVariable(String, String)
     */
    @Override
     ...
     * @param variableName the variable name
     * @return the value of the specified variable, or returns a default value if no such variable exists
     * @see org.pentaho.di.core.variables.VariableSpace#getVariable(String)
     */
    @Override
     ...
     * @see org.pentaho.di.core.variables.VariableSpace#getBooleanValueOfVariable(String, boolean)
     */
    @Override
     ...
     * @see org.pentaho.di.core.variables.VariableSpace#setVariable(String, String)
     */
    @Override
     ...
     * @see org.pentaho.di.core.variables.VariableSpace#injectVariables(Map)
     */
    @Override
    ...
    public boolean isPaused() {
        int exist = status.get() & BitMaskStatus.PAUSED.mask;
        return exist != 0;
    }

    public void setPaused( boolean paused ) {
        status.updateAndGet( v -> paused ? v | BitMaskStatus.PAUSED.mask : ( BitMaskStatus.BIT_STATUS_SUM ^ BitMaskStatus.PAUSED.mask ) & v );
    }
     ...
     * @see org.pentaho.di.core.parameters.NamedParams#addParameterDefinition(String, String,
     * String)
     */
    @Override
     ...
     * @see org.pentaho.di.core.parameters.NamedParams#getParameterDefault(String)
     */
    @Override
     ...
     * @throws UnknownParamException if the parameter does not exist
     * @see org.pentaho.di.core.parameters.NamedParams#getParameterDescription(String)
     */
    @Override
     ...
     * @throws UnknownParamException if the parameter does not exist
     * @see org.pentaho.di.core.parameters.NamedParams#getParameterValue(String)
     */
    @Override
     ...
     * @throws UnknownParamException if the parameter does not exist
     * @see org.pentaho.di.core.parameters.NamedParams#setParameterValue(String, String)
     */
    @Override
        ...
        heartbeat.scheduleAtFixedRate( new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if ( MyTrans.this.isFinished() ) {
                        log.logBasic( "Shutting down heartbeat signal for " + getName() );
                        shutdownHeartbeat( MyTrans.this.heartbeat );
                        return;
                    }

                    log.logDebug( "Triggering heartbeat signal for " + getName() + " at every " + intervalInSeconds
                            + " seconds" );
                    ExtensionPointHandler.callExtensionPoint( log, KettleExtensionPoint.TransformationHeartbeat.id, MyTrans.this );
